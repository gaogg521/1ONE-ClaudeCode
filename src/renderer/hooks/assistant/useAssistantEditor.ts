...
export const useAssistantEditor = ({
  ...
  const [pendingDeleteAssistant, setPendingDeleteAssistant] = useState<AssistantListItem | null>(null);
  ...
  const handleDeleteClick = () => {
    ...
    setPendingDeleteAssistant(activeAssistant);
    setDeleteConfirmVisible(true);
  };

  // Delete directly from list (without opening the editor drawer)
  const handleDirectDeleteClick = (assistant: AssistantListItem) => {
    if (isExtensionAssistant(assistant)) return;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // Only close drawer if the deleted assistant was the one being edited
      if (activeAssistant?.id === target.id) {
        setEditVisible(false);
      }
