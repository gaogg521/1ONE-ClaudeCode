  ...
  const hookColumns = [
    {
      ...
      width: 160,
      render: (v: string) => (
        <Tag color={EVENT_COLORS[v] ?? 'gray'} size='small'>
          {v}
        </Tag>
      ),
    },
    ...
    },
    {
      ...
      width: 80,
      render: (v: string) => (
        <Tag color={v === 'user' ? 'purple' : 'green'} size='small'>
          {v === 'user' ? '全局' : '项目'}
        </Tag>
      ),
    },
        ...
        <Form layout='vertical' className='space-y-12px'>
          <Form.Item label='事件类型'>
            <AionSelect value={form.event} onChange={(v) => setForm((f) => ({ ...f, event: v }))}>
              {HOOK_EVENTS.map((e) => (
                <AionSelect.Option key={e} value={e}>
                  <Tag color={EVENT_COLORS[e] ?? 'gray'} size='small'>
                    {e}
                  </Tag>
                </AionSelect.Option>
              ))}
          ...
          <Form.Item label='匹配器 (可选)' extra='留空 = 匹配所有工具，或填写工具名如 Bash'>
            <Input
              placeholder='例如: Bash'
              value={form.matcher}
              onChange={(v) => setForm((f) => ({ ...f, matcher: v }))}
            />
          </Form.Item>
          <Form.Item label='处理器命令' required extra='Python/Shell 脚本路径'>
            <Input
              placeholder='例如: python ~/.claude/hooks/notify.py'
              value={form.command}
