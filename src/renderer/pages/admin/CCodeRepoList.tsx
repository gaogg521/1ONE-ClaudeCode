...
import { createCodeRepo, deleteCodeRepo, listCodeRepos, type CodeRepo } from '@/renderer/utils/enterpriseApi/modules';
...
const CCodeRepoList: React.FC = () => {
  ...
  const columns = [
    { title: '名称', dataIndex: 'name' },
    {
      ...
      dataIndex: 'url',
      render: (v: string) => (
        <span className='text-12px font-mono text-t-secondary truncate' style={{ maxWidth: 200 }}>
          {v}
        </span>
      ),
    },
    {
      ...
      dataIndex: 'provider',
      render: (v: string) => (
        <Tag color={v === 'gitlab' ? 'orange' : v === 'github' ? 'arcoblue' : 'gray'}>{PROVIDERS[v] || v}</Tag>
      ),
    },
  ...
  return (
    <AdminPageWrapper>
      ...
      <Card bordered={false} className='rd-12px'>
        ...
        >
          <Table data={reposState.data} rowKey='id' columns={columns} pagination={false} size='small' border={false} />
        </ModuleDataState>
      </Card>
      <Modal
        title='绑定代码仓库'
      ...
      >
        <Form layout='vertical'>
          <Form.Item label='名称' required>
            <Input value={form.name} onChange={(v) => setForm((s) => ({ ...s, name: v }))} placeholder='1ONE-Main' />
          </Form.Item>
          <Form.Item label='仓库 URL' required>
            <Input
              value={form.url}
              onChange={(v) => setForm((s) => ({ ...s, url: v }))}
              placeholder='https://gitlab.com/team/1one.git'
            />
          </Form.Item>
          <Form.Item label='平台'>
            <Select value={form.provider} onChange={(v) => setForm((s) => ({ ...s, provider: String(v) }))}>
          ...
          <Form.Item label='默认分支'>
            <Input value={form.default_branch} onChange={(v) => setForm((s) => ({ ...s, default_branch: v }))} />
          </Form.Item>
          <ResourceScopeFields
            scope={resourceScope.scope}
