...
export const useGuidAgentSelection = ({
  ...
  useEffect(() => {
    ...
    const loadLastSelectedAgent = async () => {
      try {
        ...
        // No saved preference — prefer Claude Code, then first available agent
        const claudeAgent = availableAgents.find((a) => a.backend === 'claude');
        const preferredAgent = claudeAgent ?? availableAgents[0];
        if (preferredAgent) {
          const preferredKey = getAgentKey(preferredAgent);
          _setSelectedAgentKey(preferredKey);
        }
      } catch (error) {
  ...
  // Key of the first non-preset CLI agent (prefer Claude Code, used as fallback when leaving preset mode)
  const defaultAgentKey = useMemo(() => {
    const claudeAgent = availableAgents?.find((a) => a.backend === 'claude' && !a.isPreset);
    const firstCliAgent = claudeAgent ?? availableAgents?.find((a) => !a.isPreset);
