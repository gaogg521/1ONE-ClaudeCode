...
const QuickActionButtons: React.FC<QuickActionButtonsProps> = ({ onOpenLink: _onOpenLink, inactiveBorderColor, activeShadow }) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [hoveredWebui, setHoveredWebui] = useState(false);
  ...
  const webuiStyle = {
    ...
    boxShadow: hoveredWebui ? activeShadow : 'none',
  };
  ...
  return (
    <div
      className={`absolute right-24px flex flex-col items-end gap-6px ${styles.guidQuickActions}`}
    >
      <div className='text-right text-12px leading-relaxed' style={{ color: 'var(--color-text-4)' }}>
        <div>欢迎使用，如有任何问题可联系作者</div>
        <div>QQ：475332294 &nbsp; 姓名：Allenzhao</div>
      </div>
      <div
        ...
        style={webuiStyle}
        onMouseEnter={() => setHoveredWebui(true)}
        onMouseLeave={() => setHoveredWebui(false)}
