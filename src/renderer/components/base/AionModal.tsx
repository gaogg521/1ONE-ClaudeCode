...
const AionModal: React.FC<AionModalProps> = ({
      ...
      <div className={headerClassName} style={headerStyle}>
        ...
        {headerConfig.showClose && (
          <button onClick={onCancel} className={CLOSE_BUTTON_CLASS} aria-label='Close'>
            {headerConfig.closeIcon || <Close size={20} fill='var(--color-text-3)' />}
          </button>
        )}
      </div>
    );
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={`one-modal ${className}`}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div className='one-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='one-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
