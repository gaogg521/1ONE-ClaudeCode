...
export const CSRF_COOKIE_NAME = 'one-csrf-token';
 ...
 * 建议生产环境配置 HTTPS 并设置 ONE_HTTPS=true
 * Recommend configuring HTTPS in production and setting ONE_HTTPS=true
 */
export function getCookieOptions(): {
  ...
  const isHttps =
    process.env.ONE_HTTPS === 'true' || (process.env.NODE_ENV === 'production' && process.env.HTTPS === 'true');
...
export const SECURITY_CONFIG = {
  HEADERS: {
    ...
    CSP_DEV:
      "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://lf-package-cn.feishucdn.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' ws: wss: blob: https://open.feishu.cn https://accounts.feishu.cn https://passport.feishu.cn; frame-src 'self' https://passport.feishu.cn; media-src 'self' blob:;",
    // 生产环境 CSP（Content-Security-Policy for production）
    CSP_PROD:
      "default-src 'self'; script-src 'self' 'unsafe-inline' https://lf-package-cn.feishucdn.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' ws: wss: blob: https://open.feishu.cn https://accounts.feishu.cn https://passport.feishu.cn; frame-src 'self' https://passport.feishu.cn; media-src 'self' blob:;",
  },
  CSRF: {
