...
class TokenExtractor {
   ...
   * 2. Cookie (1ONE ClaudeCode-session)
   *
...
export const TokenMiddleware = {
  ...
  extractWebSocketToken(req: IncomingMessage): string | null {
    ...
    if (typeof cookieHeader === 'string') {
      ...
      const cookieToken = cookies[AUTH_CONFIG.COOKIE.NAME] ?? cookies['1one-claudecode-session'];
      if (typeof cookieToken === 'string') return normalizeToken(cookieToken);
    }
