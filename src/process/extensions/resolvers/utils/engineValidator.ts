...
/**
 * Current 1ONE ClaudeCode extension API version.
 * Increment major for breaking changes, minor for new features, patch for fixes.
 */
export const ONE_VERSION = getOneCmdVersion();
...
/**
 * Get the 1ONE ClaudeCode version from package.json.
 * Falls back to '0.0.0' if not available.
 */
function getOneCmdVersion(): string {
...
/**
 * Validate that an extension's engine requirements are satisfied by the current 1ONE ClaudeCode version.
 *
 * Checks:
 * 1. engine.oneCmd — does the running 1ONE ClaudeCode version satisfy the required range?
...
export function validateEngineCompatibility(extension: LoadedExtension): EngineValidationResult {
  ...
  // Check 1ONE ClaudeCode core version compatibility
  if (engine?.oneCmd) {
    if (!satisfiesVersion(ONE_VERSION, engine.oneCmd)) {
      result.valid = false;
      result.issues.push(
        `Extension "${extension.manifest.name}" requires 1ONE ClaudeCode ${engine.oneCmd} but current version is ${ONE_VERSION}`
      );
    }
  }
