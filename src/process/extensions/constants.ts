 ...
 *
 * Priority order:
 *   1. Environment variable (`ONE_EXTENSIONS_PATH`) — highest
 *   2. User data dir (`~/.1ONE ClaudeCode/extensions`)
 *   3. Electron appData dir
 *
 * E2E test mode (`ONE_E2E_TEST=1`) only scans env dirs for hermetic runs.
 */
export function getExtensionScanSources(): ExtensionScanSource[] {
  const sources: ExtensionScanSource[] = [];
  const seen = new Set<string>();
  const isE2ETest = process.env.ONE_E2E_TEST === '1';
