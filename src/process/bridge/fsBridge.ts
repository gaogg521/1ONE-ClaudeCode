...
export function initFsBridge(): void {
  ...
  ipcBridge.fs.createTempFile.provider(async ({ fileName }) => {
    try {
      ...
      if (fileExists) {
        ...
        const tempFileName = `${name}${ONE_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
      ...
      for (const filePath of filePaths) {
        try {
          ...
          if (exists) {
            ...
            const newFileName = `${name}${ONE_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  // Skills Market: inject the 1ONE ClaudeCode-skills builtin skill
  ...
  // Skills Market: remove the 1ONE ClaudeCode-skills builtin skill
...
/**
 * Read the bundled SKILL.md for 1ONE ClaudeCode-skills from app resources.
 *
