...
const DEFAULT_REPO = 'gaogg521/1ONE-Claude-Code';
const DEFAULT_USER_AGENT = '1one';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.ONE_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `1ONE-update-${Date.now()}`;
};
