...
4. **Option B: Start 1ONE WebUI Remote Mode**
   - **Must use 1ONE-webui-setup skill**: Consult `references/1ONE-webui.md`
   - **Workflow**:
     ...
     2. After user replies, **guide user to 1ONE settings interface**:
        ...
        - **Configuration steps**: Follow `1ONE-webui-setup` skill's `references/1ONE-webui.md` documentation to guide user:
   ...
   - **Key principles**:
     ...
     - **Don't attempt to install `@1ONE/webui` or similar npm packages**: WebUI is a built-in feature of 1ONE, not a separate package
...
### 1ONE-webui-setup Skill

**Core documentation**: `references/1ONE-webui.md`

**When to use**: When user chooses WebUI option, use immediately

**How to use**:

1. **Directly consult `references/1ONE-webui.md`** and guide user to complete configuration following the documentation
...
3. **Choose WebUI**: Use `1ONE-webui-setup` skill → Ask needs → Choose solution → Execute configuration → Provide usage instructions
...
7. **Skill usage**:
   ...
   - WebUI configuration → **Must use `1ONE-webui-setup` skill** (directly consult `references/1ONE-webui.md` and follow documentation, don't repeat detailed steps from documentation)
