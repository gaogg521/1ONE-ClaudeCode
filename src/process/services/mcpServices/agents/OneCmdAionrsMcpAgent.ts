 ...
 * Maps to 1ONE ClaudeCode transport types (snake_case)
 */
...
/**
 * Map aionrs transport type (kebab-case) to 1ONE ClaudeCode transport type
 */
function toOneCmdTransportType(aionrsType: AionrsTransportType): IMcpServerTransport['type'] {
  if (aionrsType === 'streamable-http') return 'streamable_http';
  return aionrsType;
}

/**
 * Map 1ONE ClaudeCode transport type to aionrs transport type (kebab-case)
 */
...
/**
 * Convert an aionrs server config entry to an 1ONE ClaudeCode IMcpServer
 */
function toMcpServer(name: string, config: AionrsServerConfig): IMcpServer {
  const transportType = toOneCmdTransportType(config.transport);
  const now = Date.now();
...
/**
 * Convert an 1ONE ClaudeCode IMcpServer to an aionrs server config entry
 */
...
export class AionrsMcpAgent extends AbstractMcpAgent {
  ...
  getSupportedTransports(): string[] {
    // aionrs supports stdio, sse, streamable-http (mapped to streamable_http in 1ONE ClaudeCode)
