 ...
 * - '1one': @office-ai/aioncli-core (1ONE ClaudeCode 本地管理的 Gemini 实现)
 */
export class McpService {
  ...
  private getAgentForConfig(agent: { backend: AcpBackend; cliPath?: string }): IMcpProtocol | undefined {
    // Fork Gemini 使用 OneCmdMcpAgent 管理 MCP 配置
    // Fork Gemini uses OneCmdMcpAgent to manage MCP config
    if (agent.backend === 'gemini' && !agent.cliPath) {
      return this.agents.get('1one');
    }
   ...
   * Fork Gemini (backend='gemini', no cliPath) uses OneCmdMcpAgent.
   */
