# 1ONE Database System

本文档介绍 1ONE 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/1ONE.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/1ONE/`
- Windows: `%APPDATA%/1ONE/`
- Linux: `~/.config/1ONE/`
...
直接复制 `1ONE.db` 和 `1ONE.db-wal` 文件即可。
