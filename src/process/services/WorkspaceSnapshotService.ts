...
export class WorkspaceSnapshotService {
  ...
  /**
   * Remove leftover `1ONE-snapshot-*` directories from previous sessions
  ...
  static async cleanupStaleSnapshots(): Promise<void> {
    ...
    const stale = entries.filter((name) => name.startsWith('one-snapshot-'));
  ...
  private async createWorkingTreeSnapshot(workspacePath: string): Promise<string> {
    const gitdir = path.join(os.tmpdir(), `1ONE-snapshot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    await execFileAsync(
      ...
      [
        ...gitArgs,
        '-c',
        'user.name=1ONE ClaudeCode',
        '-c',
        'user.email=snapshot@1ONE ClaudeCode.local',
        'commit',
        '--allow-empty',
        '-m',
        'baseline',
