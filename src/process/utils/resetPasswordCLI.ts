...
export async function resetPasswordCLI(username: string): Promise<void> {
  try {
    ...
    const dbPath = path.join(getDataPath(), '1one.db');
    ...
    if (!hasUsersResult.data) {
      ...
      log.info('');
      log.info('Please run 1ONE ClaudeCode at least once to initialize the database:');
      log.info('  1ONE ClaudeCode --webui');
      log.info('');
      log.info('Then you can reset the password using:');
      log.info('  1ONE ClaudeCode --resetpass <username>');
      process.exit(1);
    }
