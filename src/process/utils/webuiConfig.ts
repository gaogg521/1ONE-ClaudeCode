...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.ONE_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.ONE_ALLOW_REMOTE || process.env.ONE_REMOTE);
  const hostHint = process.env.ONE_HOST?.trim();
