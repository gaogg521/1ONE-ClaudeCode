...
export function getElectronConfigCandidatePaths(): string[] {
  ...
  if (process.platform === 'darwin') {
    return [
      path.join(home, '.1ONE ClaudeCode-config', 'one-config.txt'),
      path.join(home, '.1ONE ClaudeCode-config-dev', 'one-config.txt'),
    ];
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(home, 'AppData', 'Roaming');
    return [
      path.join(appData, '1one', 'config', 'one-config.txt'),
      path.join(appData, 'one-Dev', 'config', 'one-config.txt'),
    ];
  }
  // Linux and other platforms
  return [
    path.join(home, '.config', '1one', 'config', 'one-config.txt'),
    path.join(home, '.config', 'one-Dev', 'config', 'one-config.txt'),
  ];
}
...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    console.log('[1ONE ClaudeCode] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[1ONE ClaudeCode] Config migration from Electron failed:', error);
  }
}
 ...
 * @param sourcePath - absolute path to an 1ONE ClaudeCode-config.txt file
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[1ONE ClaudeCode] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[1ONE ClaudeCode] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[1ONE ClaudeCode] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[1ONE ClaudeCode] IMPORT_CONFIG_FROM failed:', error);
  }
}
