 ...
 * Release builds use ~/.1ONE ClaudeCode; dev builds use ~/.1ONE ClaudeCode-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.1ONE ClaudeCode，Dev 模式使用 ~/.1ONE ClaudeCode-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, '1one');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.1ONE ClaudeCode'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.1ONE ClaudeCode-config; dev builds use ~/.1ONE ClaudeCode-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.1ONE ClaudeCode-config，Dev 模式使用 ~/.1ONE ClaudeCode-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.1ONE ClaudeCode-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[1ONE ClaudeCode] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[1ONE ClaudeCode] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[1ONE ClaudeCode] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[1ONE ClaudeCode] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
