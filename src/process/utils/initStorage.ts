...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[1ONE ClaudeCode] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[1ONE ClaudeCode] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[1ONE ClaudeCode] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('1one.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'one-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'one-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'one-chat-history', conversation_id + '.txt'));
};
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    console.warn(`[1ONE ClaudeCode] Could not find builtin ${dirPath} directory, tried:`, candidates);
    return candidates[0];
  };
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            console.warn(`[1ONE ClaudeCode] Source rule file not found: ${sourceRulesPath}`);
            continue;
          }
          ...
          console.warn(`[1ONE ClaudeCode] Failed to copy rule file ${ruleFile}:`, error);
        }
      }
    } else {
      ...
      for (const [locale, skillFile] of Object.entries(preset.skillFiles)) {
        try {
          ...
          if (!existsSync(sourceSkillsPath)) {
            console.warn(`[1ONE ClaudeCode] Source skill file not found: ${sourceSkillsPath}`);
            continue;
          }
          ...
          console.warn(`[1ONE ClaudeCode] Failed to copy skill file ${skillFile}:`, error);
        }
      }
    } else {
...
const ensureBuiltinMcpServers = async (): Promise<void> => {
  try {
    ...
    const buildEnvFromConfig = (cfg: typeof oldConfig): Record<string, string> => {
      ...
      if (cfg.platform) env.ONE_IMG_PLATFORM = cfg.platform;
      if (cfg.baseUrl) env.ONE_IMG_BASE_URL = cfg.baseUrl;
      if (cfg.apiKey) env.ONE_IMG_API_KEY = cfg.apiKey;
      if (cfg.useModel) env.ONE_IMG_MODEL = cfg.useModel;
      return env;
    };
    ...
    console.error('[1ONE ClaudeCode] Failed to ensure built-in MCP servers:', error);
  }
};
...
const cleanupOrphanedHealthCheckConversations = async () => {
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[1ONE ClaudeCode] Cleaned up ${deletedCount} orphaned health-check conversation(s) on startup`);
    }
  } catch (error) {
    console.warn('[1ONE ClaudeCode] Failed to cleanup orphaned health-check conversations:', error);
  }
};
...
const initStorage = async () => {
  ...
  try {
    ...
    console.error('[1ONE ClaudeCode] Failed to initialize default MCP servers:', error);
  }
