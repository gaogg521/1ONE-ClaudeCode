...
export async function loadCliConfig({
  ...
  const builtinSkillsExtension: GeminiCLIExtension = {
    name: 'one-builtin-skills',
    version: '1.0.0',
    isActive: true,
    path: skillsDir || '',
    contextFiles: [],
    id: 'one-builtin-skills',
  ...
  const config = new Config({
    ...
    // 1ONE 是桌面应用，支持用户交互确认，需要设置 interactive: true
    // 1ONE is a desktop app with user interaction support, needs interactive: true
    interactive: true,
