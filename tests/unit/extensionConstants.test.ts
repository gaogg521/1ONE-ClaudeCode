...
vi.mock('@process/utils', () => ({
  getDataPath: () => '/home/user/.1one-claudecode-dev',
}));
...
describe('extension constants', () => {
  ...
  describe('getUserExtensionsDir', () => {
    it('should return data path + extensions', () => {
      expect(getUserExtensionsDir()).toBe(path.join('/home/user/.1one-claudecode-dev', 'extensions'));
    });
  });
  ...
  describe('getEnvExtensionsDirs', () => {
    const originalEnv = process.env.ONE_EXTENSIONS_PATH;

    afterEach(() => {
      if (originalEnv === undefined) {
        delete process.env.ONE_EXTENSIONS_PATH;
      } else {
        process.env.ONE_EXTENSIONS_PATH = originalEnv;
      }
    });
    ...
    it('should split env var by path separator', () => {
      ...
      process.env.ONE_EXTENSIONS_PATH = `/path/a${sep}/path/b`;
      expect(getEnvExtensionsDirs()).toEqual(['/path/a', '/path/b']);
    });

    it('should filter out empty segments', () => {
      const sep = process.platform === 'win32' ? ';' : ':';
      process.env.ONE_EXTENSIONS_PATH = `/path/a${sep}${sep}/path/b`;
  ...
  describe('getHubResourcesDir', () => {
    ...
    it('should return process resources path in packaged mode', () => {
      ...
      Object.defineProperty(process, 'resourcesPath', {
        value: '/opt/1ONE ClaudeCode/resources',
        configurable: true,
        writable: true,
      });

      expect(getHubResourcesDir()).toBe(path.join('/opt/1ONE ClaudeCode/resources', 'hub'));
    });
  });

  describe('getExtensionScanSources', () => {
    const originalEnv = process.env.ONE_EXTENSIONS_PATH;
    const originalE2E = process.env.ONE_E2E_TEST;

    afterEach(() => {
      if (originalEnv === undefined) delete process.env.ONE_EXTENSIONS_PATH;
      else process.env.ONE_EXTENSIONS_PATH = originalEnv;
      if (originalE2E === undefined) delete process.env.ONE_E2E_TEST;
      else process.env.ONE_E2E_TEST = originalE2E;
    });

    it('should return user and appdata dirs by default', () => {
      delete process.env.ONE_EXTENSIONS_PATH;
      delete process.env.ONE_E2E_TEST;
    ...
    it('should include env dirs with highest priority when set', () => {
      process.env.ONE_EXTENSIONS_PATH = '/custom/ext';
      delete process.env.ONE_E2E_TEST;
      const sources = getExtensionScanSources();
    ...
    it('should only include env dirs in E2E mode', () => {
      process.env.ONE_EXTENSIONS_PATH = '/e2e/ext';
      process.env.ONE_E2E_TEST = '1';
      const sources = getExtensionScanSources();
  ...
  describe('getInstallTargetDir', () => {
    ...
    it('should return env dir when ONE_EXTENSIONS_PATH is set', () => {
      process.env.ONE_EXTENSIONS_PATH = '/my/ext';
      delete process.env.ONE_E2E_TEST;
      const dir = getInstallTargetDir();
