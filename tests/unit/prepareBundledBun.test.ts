...
describe('prepareBundledBun', () => {
  ...
  const originalCacheDir = process.env.ONE_BUN_CACHE_DIR;
  const originalVersion = process.env.ONE_BUN_VERSION;
  ...
  afterEach(() => {
    process.env.ONE_BUN_CACHE_DIR = originalCacheDir;
    process.env.ONE_BUN_VERSION = originalVersion;
  ...
  it('copies bundled bun from cache when cache metadata is valid', () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), '1one-claudecode-bun-test-'));
    ...
    process.env.ONE_BUN_CACHE_DIR = cacheRoot;
    process.env.ONE_BUN_VERSION = version;
