...
const testState = vi.hoisted(() => ({
  ...
  initialBuiltinServer: (): MockServer => ({
    ...
    transport: {
      ...
      env: {
        ONE_IMG_PLATFORM: 'new-api',
        ONE_IMG_BASE_URL: 'https://example.com',
        ONE_IMG_API_KEY: 'key',
        ONE_IMG_MODEL: 'grok-imagine-1.0',
      },
    },
  }),
  mockConfigGet: vi.fn(),
...
describe('ToolsModalContent image generation status refresh', () => {
  ...
  it('refreshes image generation agent status only after sync completes when enabling the builtin MCP server', async () => {
    ...
    expect(testState.mockCheckSingleServerInstallStatus).toHaveBeenCalledWith('1one-claudecode-image-generation');
  });
