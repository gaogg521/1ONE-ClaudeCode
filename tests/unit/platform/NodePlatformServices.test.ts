...
describe('NodePlatformServices.paths', () => {
  ...
  it('getDataDir falls back to homedir/.1one-claudecode-server', async () => {
    const { NodePlatformServices } = await import('../../../src/common/platform/NodePlatformServices');
    expect(new NodePlatformServices().paths.getDataDir()).toBe(path.join(os.homedir(), '.1one-claudecode-server'));
  });
