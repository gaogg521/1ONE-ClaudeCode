    ...
    expect(result).toEqual([
      {
        ...
        args: ['/abs/builtin-mcp-image-gen.js'],
        env: [
          { name: 'ONE_IMG_PLATFORM', value: 'openai' },
          { name: 'ONE_IMG_MODEL', value: 'gpt-image-1' },
        ],
      },
      {
        type: 'http',
...
const mockUnrelatedMcpAgents = (emptyDetect: () => Promise<IMcpServer[]>) => {
  ...
  vi.doMock('../../src/process/services/mcpServices/agents/OneCmdMcpAgent', () => ({
    OneCmdMcpAgent: makeAgentClass(emptyDetect),
  }));
};
...
describe('McpService Gemini detection', () => {
  ...
  it('reports built-in Gemini MCP servers under gemini source', async () => {
    ...
    vi.doMock('../../src/process/services/mcpServices/agents/OneCmdMcpAgent', () => ({
      OneCmdMcpAgent: makeAgentClass(builtinDetect),
    }));
