...
describe('deepLink module', () => {
  ...
  describe('parseDeepLinkUrl', () => {
    it('should parse simple deep link URL', async () => {
      ...
      const result = parseDeepLinkUrl('1one-claudecode://add-provider?baseUrl=http://localhost&apiKey=sk-123');
    ...
    it('should parse deep link with path segments', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
      const result = parseDeepLinkUrl('1one-claudecode://provider/add?v=1');
    ...
    it('should decode base64 data param and merge into params', async () => {
      ...
      const result = parseDeepLinkUrl(`1one-claudecode://provider/add?v=1&data=${data}`);
    ...
    it('should handle invalid base64 data gracefully', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
      const result = parseDeepLinkUrl('1one-claudecode://add?data=not-valid-base64!!!');

      expect(result).not.toBeNull();
      expect(result!.params.data).toBeUndefined();
    });

    it('should return null for non-1one-claudecode protocol', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
  ...
  describe('handleDeepLinkUrl', () => {
    it('should queue URL when no window is set', async () => {
      ...
      handleDeepLinkUrl('1one-claudecode://test-action?key=val');

      expect(getPendingDeepLinkUrl()).toBe('1one-claudecode://test-action?key=val');
    });
  ...
  describe('pending URL state', () => {
    it('should clear pending URL', async () => {
      ...
      handleDeepLinkUrl('1one-claudecode://test');
      expect(getPendingDeepLinkUrl()).toBe('1one-claudecode://test');
