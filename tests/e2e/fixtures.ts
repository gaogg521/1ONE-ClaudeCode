 ...
 *      (e.g. out/linux-unpacked/1one-claudecode, out/mac-arm64/1ONE ClaudeCode.app, out/win-unpacked/1ONE ClaudeCode.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), '1one-claudecode-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/1ONE ClaudeCode.exe  or  out/win-x64-unpacked/1ONE ClaudeCode.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, '1ONE ClaudeCode.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/1ONE ClaudeCode.app/Contents/MacOS/1ONE ClaudeCode  or  out/mac/1ONE ClaudeCode.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', '1ONE ClaudeCode');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/1one-claudecode  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['1one-claudecode', '1ONE ClaudeCode']) {
