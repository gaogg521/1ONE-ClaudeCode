...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), '1one-claudecode-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), '1one-claudecode-e2e-no-extensions-state-'));
