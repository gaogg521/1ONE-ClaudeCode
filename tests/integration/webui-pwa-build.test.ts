...
const pwaSourceFiles = [
  ...
  path.resolve(repoRoot, 'src/renderer/services/registerPwa.ts'),
  path.resolve(repoRoot, 'public/manifest.webmanifest'),
...
runOrSkip('copies PWA assets into renderer output and packaged app.asar', () => {
  ...
  for (const relativeAsset of expectedAssets) {
    const absoluteAsset = path.resolve(repoRoot, relativeAsset);
    expect(fs.existsSync(absoluteAsset), `${relativeAsset} should exist`).toBe(true);
  }

  const manifestPath = path.resolve(repoRoot, 'out/renderer/manifest.webmanifest');
