...
# Check open PRs
gh pr list --repo iOfficeAI/1ONE ClaudeCode --state open --search "<error-keyword>" --json number,title

# Check closed PRs (recently merged)
gh pr list --repo iOfficeAI/1ONE ClaudeCode --state merged --search "<error-keyword>" --limit 5 --json number,title,mergedAt
```
