...
```
~/.1one-claudecode-fix-issues/skip-list.json
```
...
```bash
gh api repos/iOfficeAI/1ONE ClaudeCode/issues/<number>/comments \
...
```bash
gh issue list --repo iOfficeAI/1ONE ClaudeCode --state open --label bug --limit 50 \
...
gh api repos/iOfficeAI/1ONE ClaudeCode/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo iOfficeAI/1ONE ClaudeCode --body "$(cat <<'EOF'
...
```bash
gh pr list --repo iOfficeAI/1ONE ClaudeCode --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.1one-claudecode-fix-issues/skip-list.json`
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | iOfficeAI/1ONE ClaudeCode | GitHub repository (owner/repo)                                      |
