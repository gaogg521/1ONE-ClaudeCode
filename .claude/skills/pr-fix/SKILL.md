...
PR_NUMBER=<PR_NUMBER>
WORKTREE_DIR="/tmp/1one-claudecode-pr-${PR_NUMBER}"
...
2. Pre-flight: fetch PR info (state, isCrossRepository, maintainerCanModify, forkOwner)
   → ABORT: state=MERGED
3. Create worktree at /tmp/1one-claudecode-pr-<PR_NUMBER>:
...
9. Cleanup: git worktree remove /tmp/1one-claudecode-pr-<PR_NUMBER>
```
