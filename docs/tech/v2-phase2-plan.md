...
## 接手状态（2026-07-06 14:xx 更新，下一会话从这里继续）
...
**本轮追加完成（09:10 之后，均已提交 one-main）**：
- **A4 测试计划 + CI 流水线域** ✅（AionCore `6c398e6` 10:59 + AionUi `847c75a` 10:59）：one-devops 新增 test plans 与 pipelines(CI) 两域（增表 + migration + CRUD/路由 + 前端 UI）。**A4 仅剩 value stream 价值流域未做**（需求驱动按需）。
- **B3 fork 品牌化** ✅（AionUi `e41a804` 11:46，原 D3 决策「推迟」已提前落地）：fork 全量 rebrand 为 1ONE Code（appId/productName/图标/用户可见文案）。⚠️ 改动**仅在源码，未重打包**——出新 Windows 安装包前须先 bump version 且不删任何旧 .exe（见 memory `feedback-build-artifacts`）。回归 userData 路径与 one-import 源定位待重打包验证。
- **C 组飞书 SSO + LDAP 真实环境 E2E** ✅（配在**原版 1one-command**，非 fork）：飞书真人扫码登录成功 + app 内「测试连接」过；LDAP 域控完整 3 步（服务绑定→搜索→用户密码绑定）通过，用服务账号 `ldap` 本身作真实测试用户绕过 D5。排查 facts/参数见 memory `feishu-sso-e2e-verified.md`。**C1 飞书、C3 LDAP 收官**。

**fork 现状（HEAD，均已提交 one-main）**：AionCore = `6c398e6`；AionUi = `4c5ec67`（i18n rebrand `317a327` + B1 后端源默认指向 fork AionCore `4c5ec67`，原 dirty 的 package.json/prepare-aioncore.js 已提交）。
...
**剩余待办（按建议优先级）**：
1. **重打包收尾**（明确动作）——B3 rebrand + i18n(551处) + A4 新域 + B1 后端源默认切 fork 都在源码里但未出安装包。出新 Windows 包前：先 bump `package.json` version（patch+1）→ commit/push → `AIONUI_BACKEND_LOCAL_PATH=...aioncore.exe bun run dist:win`；**一个旧 .exe 都不能删**（见 memory `feedback-build-artifacts`）。顺带回归 rebrand 后 userData 路径与 one-import 源定位。
2. **钉钉 / 企业微信 SSO**——🅿️ **用户 2026-07-06 明确暂缓**（非漏做），等以后拿到真实凭据再完成并回来更新文档。代码路径已就绪，仅缺配置。redirectUri 模板 `http://192.168.11.137:25809/api/auth/{dingtalk|wecom}/callback`（需加各自开放平台白名单）。字段：钉钉 appKey/appSecret/corpId(可选)；企微 corpId/agentId/secret。
3. **C 组跨用户活体 E2E（多用户企业组织）**——⛔ 卡 D5 多用户环境。**A1 L3 团队共享的跨用户验证**（A 共享员工、B 同租户派活成功、跨租户拒绝）本轮已用单测+tenant 解析 E2E 组合覆盖，真实多用户验证归此轮；C2 M4d OAuth 302 真跳转同此环境。
4. **A4 value stream 价值流域**——需求驱动按需做。入口同 one-devops 增表 + migration（⚠️ 新增 sqlx 内存库测试必须 `max_connections(1)`）。
5. **A1 L3 存量员工 tenant backfill**（可选）——本改动前建的员工 tenant='default'，如企业用户要共享它们需 backfill 到企业 tenant；新员工已带正确 tenant，非阻塞。
6. **A2 RAG 增强**（可选，非阻塞）——① 检索结果注入编排 dispatch；② 文档从 file_path 服务端读文件；③ 大语料换近似检索。
...
> fork 现状（本文档权威 HEAD）：AionCore `6c398e6` / AionUi `4c5ec67`（均在 one-main）。
...
| D3 | fork 安装包品牌（AionUi→1ONE） | ✅→**已实施**（2026-07-06 AionUi `e41a804`）。原决策为「推迟到灰度前」，本轮提前一次性做完 rebrand。⚠️ 仅源码，未重打包；userData 路径 / one-import 源定位回归待随重打包验证 | B3 ✅（仅剩重打包） |
...
| D5 | E2E 凭据与环境 | 🔶 **部分解锁**（2026-07-06）：飞书 SSO（真人扫码✅）+ LDAP 域控（真实 3 步✅）用户已提供凭据并验证通过。**仍卡**：钉钉/企微凭据、多用户企业组织环境（跨用户共享 E2E + M4d OAuth 302 真跳转） | C1 飞书✅/C3 LDAP✅；剩钉钉·企微·多用户 |
...
- **test plans / pipelines(CI)** ✅ 2026-07-06 完成（AionCore `6c398e6` + AionUi `847c75a`）：one-devops 新增两域，增表 + migration + CRUD/路由 + 前端 UI。
- **value stream 价值流域**：需求驱动、按需排期，未做。入口同 `one-devops` crate 增表 + migration。⚠️ 新增 sqlx 内存库测试必须 `max_connections(1)`。
...
| B3 | ~~fork 品牌化实施~~ | 决策 D3 | ✅ 2026-07-06 完成（AionUi `e41a804`）：fork 全量 rebrand 为 1ONE Code（appId/productName/图标/用户可见文案）。⚠️ 仅源码未重打包；userData 路径与 one-import 源定位回归待随重打包验证 |
...
| # | 项 | 前置 |
|---|---|---|
| C1 | 渠道真实配对 E2E — **飞书 ✅ 收官**（2026-07-06 真人扫码登录 + 测试连接过，配在原版 1one-command，见 memory `feishu-sso-e2e-verified.md`）；**钉钉/企微 🅿️ 用户 2026-07-06 明确暂缓（等凭据）；TG/微信仍待** | 剩余卡 D5 凭据；微信部分另需 D2 |
| C2 | M4d OAuth E2E — 飞书 OAuth 全链路已随 C1 走通；剩其它 provider 的 302 真跳转 | D5 OAuth 应用 + 多用户环境 |
| C3 | LDAP 真实目录 E2E — **✅ 收官**（2026-07-06 真实域控 `ldaps://ldaps.intranet.123u.com:636` 完整 3 步：服务绑定→搜索→用户密码绑定；服务账号 `ldap` 作测试用户绕过 D5。参数见 memory `feishu-sso-e2e-verified.md`） | ~~D5~~ 已解 |
...
- **开放项唯一入口（剩余待办 + BUG 扫描 + 三方面审计）**：`docs/tech/v2-audit-and-open-items.md`
