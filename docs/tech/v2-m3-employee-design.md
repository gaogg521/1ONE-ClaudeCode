...
> **进度**：M3a ✅ 完成（2026-07-05，fork commit 见 one-main）。M3b ✅ 完成（2026-07-05）：团队员工 run-now（`TeamSessionService::send_message_to_agent` + `get_team` 取 slot conversation_id + 轮询 `get_run_state` 直到 `active_run=None`）+ cron 30s 扫描循环（复用上游 `compute_next_run`，零上游 diff）+ 002 迁移三列（schedule/schedule_enabled/next_run_at）。M3c ✅ 完成（2026-07-05，AionUi fork commit bd3e424）：superAssistant UI 首屏——三 tab（Overview/Agents/Settings）+ 4 个 Modal（Create/Manage/Schedule/Detail）+ `personalAgent` httpBridge provider + 侧栏入口。验收：单测 6/6（M3b）+ `--local` API 链路全过（list/create/update/runNow 11.8s success/setSchedule/listRuns）+ tsc 0 错误 + oxlint 0 错误。下一步 M4（管理后台 + httpBridge 适配层 + 桌面客户端连远端）。
>
> 关键实现事实：① cron 扫描器触发后立即把 `next_run_at` 置 NULL 防重入，`execute_run` 完成后调 `recompute_next_run` 重排——重启安全（启动时 NULL 的不会被扫，但 schedule 仍在库里，下次 set_schedule 或手动触发会重排）；② 团队 run 用 `get_team` 拿 slot.conversation_id（`TeamResponse.assistants[].conversation_id`），不依赖 send_message ack；③ `CronSchedule` 不派生 serde，可 serde 的是 `CronScheduleDto`（tag="kind"），存库用 Dto JSON + `schedule_from_dto` 转换；④ team run 完成判定用 `active_run.is_none()`（3s 轮询、15min 上限），slot→conversation_id 在 `crates/aionui-team/src/types.rs` L96；⑤ M3c 设计文档原说"Overview/Agents/Settings 三个纯员工域 tab"——实际不纯（1one 三 tab 大量依赖 enterprise 域），M3c 改为只迁个人员工切片（对齐 one-employee crate 能 back 的范围），enterprise 域留给 M4；⑥ AionUi fork 的 `httpRequest` 已自动 unwrap `ApiResponse.data`，provider 返回值就是 `T` 直接；⑦ 后端 update/setSchedule 接受平铺 body（不是 `{updates:{...}}`），httpBridge 的 mapBody 要平铺；⑧ AionUi fork 的 Arco `Tabs.TabPane` 用 `title` 不是 `tab`，`Modal` 无 `width` prop。
>
