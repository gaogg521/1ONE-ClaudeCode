...
git clone https://github.com/iOfficeAI/1ONE ClaudeCode.git
cd 1ONE ClaudeCode
...
When you have two clones of the repository (e.g. `1ONE ClaudeCode` and `1ONE ClaudeCode-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `ONE_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`1ONE ClaudeCode-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.1one-claudecode-dev-2`, `~/.1one-claudecode-config-dev-2`)
...
1ONE ClaudeCode uses **electron-vite** for fast bundling:
