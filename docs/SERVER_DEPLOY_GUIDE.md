# 1ONE ClaudeCode Headless Server Deployment Guide

Deploy 1ONE ClaudeCode WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- 1ONE ClaudeCode `.deb` package from [Releases](https://github.com/iOfficeAI/1ONE ClaudeCode/releases)
...
wget https://github.com/iOfficeAI/1ONE ClaudeCode/releases/latest/download/1ONE ClaudeCode-linux-amd64.deb

# Install
sudo dpkg -i 1ONE ClaudeCode-linux-amd64.deb
...
1ONE ClaudeCode is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/1ONE ClaudeCode/start-1one-claudecode.sh`:

```bash
#!/bin/bash
# 1ONE ClaudeCode WebUI headless startup script
# Usage: ./start-1one-claudecode.sh [start|stop|restart|status]

PIDFILE="/var/run/1one-claudecode.pid"
LOGFILE="/var/log/1one-claudecode.log"
WORKDIR="$HOME"  # Change to your workspace directory

start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "1ONE ClaudeCode is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/1ONE ClaudeCode --webui --remote --no-sandbox \
        > "$LOGFILE" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "1ONE ClaudeCode started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "1ONE ClaudeCode failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "1ONE ClaudeCode is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping 1ONE ClaudeCode (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "1ONE ClaudeCode --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "1ONE ClaudeCode stopped."
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "1ONE ClaudeCode is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "1ONE ClaudeCode is not running."
...
```bash
chmod +x /opt/1ONE ClaudeCode/start-1one-claudecode.sh
```

> **Tip**: `WORKDIR` determines the directory 1ONE ClaudeCode can access for file operations. Set it to your project workspace.

---

## Remote Access

1ONE ClaudeCode WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for 1ONE ClaudeCode (Electron / Chromium Layer)
...
Create `/opt/1ONE ClaudeCode/proxy.pac`:
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/1ONE ClaudeCode --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/1ONE ClaudeCode/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: 1ONE ClaudeCode Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside 1ONE ClaudeCode WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by 1ONE ClaudeCode's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i 1ONE ClaudeCode-linux-amd64.deb`                 |
| 1ONE ClaudeCode can only access `/tmp`             | Set `WORKDIR` in the startup script to your workspace path   |
...
# 1ONE ClaudeCode 无头服务器部署指南

在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 1ONE ClaudeCode WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- 1ONE ClaudeCode `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/1ONE ClaudeCode/releases)）
...
1ONE ClaudeCode 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/1ONE ClaudeCode/start-1one-claudecode.sh`：

```bash
#!/bin/bash
# 1ONE ClaudeCode WebUI 无头启动脚本
# 用法: ./start-1one-claudecode.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "1ONE ClaudeCode 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 1ONE ClaudeCode WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "1ONE ClaudeCode 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "1ONE ClaudeCode 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "1ONE ClaudeCode 未在运行"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "正在停止 1ONE ClaudeCode (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "1ONE ClaudeCode --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "1ONE ClaudeCode 已停止。"
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "1ONE ClaudeCode 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "1ONE ClaudeCode 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
1ONE ClaudeCode WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（1ONE ClaudeCode Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/1ONE ClaudeCode/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/1ONE ClaudeCode/proxy.pac"`
...
### 第四步：1ONE ClaudeCode 内置代理（Gemini API）
...
| 1ONE ClaudeCode 只能访问 /tmp   | 修改启动脚本中的 `WORKDIR`            |
